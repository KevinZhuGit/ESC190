...
PlayerRecord *create_leaf_record(Player *p)
{
	...
	*/
	...
	record->player = p; // check if right
...
PlayerRecord *add_match(PlayerRecord *p1, PlayerRecord *p2, int p1_wins, int p2_wins)
{
	...
	*/
	...
	// ODD #PLAYERS IN LATER ROUNDS???

	PlayerRecord *record = (PlayerRecord *)malloc(sizeof(PlayerRecord));

	if (p1_wins == 13)
	{
		record->player = p1->player;
		record->left_child = p2;
		record->right_child = p1;
		record->game_records[LOSSES] = p2_wins;
	}
	else
	{
		record->player = p2->player;
		record->left_child = p1;
		record->right_child = p2;
		record->game_records[LOSSES] = p1_wins;
	}

	record->parent = NULL;
	record->game_records[WINS] = 13;
...
int get_player_rank(char player_id[], PlayerRecord *root)
{
	...
	*/
	...
	// check left subtree, then right subtree to see if player is in there
	// cannot return values yet

	if (root == NULL)
	{
		return 0;
	}
	else if (strcmp(root->player->id, player_id) == 0)
	{
		return 1;
	}

	if (get_player_rank(player_id, root->left_child) >= 1)
	{
		return get_player_rank(player_id, root->left_child) + 1;
	}
	else if (get_player_rank(player_id, root->right_child) >= 1)
	{
		return get_player_rank(player_id, root->right_child) + 1;
	}
...
void clear_tournament_records_helper(PlayerRecord *curr, PlayerRecord ***records, int *num_records, int *max_records)
{
	/**
